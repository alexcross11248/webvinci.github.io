			...
			modelService.getCertificateList(data).then(function(res) {
				...
				if(res.code == 0) {
					...
					$scope.certificateList = _.map(commonService.translateServerData(res.body), function(item) {
						item.BirthDate = commonService.str2date(item.BirthDate, 'yyyy-MM-dd');
						item.FirstRegisterDate = commonService.str2date(item.FirstRegisterDate, 'yyyy-MM-dd');
						item.LastRegisterDate = commonService.str2date(item.LastRegisterDate, 'yyyy-MM-dd');
						item.RegisterToDate = commonService.str2date(item.RegisterToDate, 'yyyy-MM-dd');
						item.CertificateDate = commonService.str2date(item.CertificateDate, 'yyyy-MM-dd');
						item.FirstJobTime = commonService.str2date(item.FirstJobTime, 'yyyy-MM-dd');
						item.Picture1 = modelService.rootUrl + 'PC/' + item.Picture1;
						item.Picture2 = modelService.rootUrl + 'PC/' + item.Picture2;
						item.Picture3 = modelService.rootUrl + 'PC/' + item.Picture3;
